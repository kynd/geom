...
// All shapes share a slow Y-axis rotation and the current time t.
// Static shapes (1–10) ignore t except for rotation.
// Moving shapes (11–30) use t for internal animation.
...
float surfaceF(vec3 p) {
  return mix(shapeFunc(p, u_shapeA), shapeFunc(p, u_shapeB), u_blend);
}
