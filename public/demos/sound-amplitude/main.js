...
async function init() {
  ...
  const renderPass = new RenderPass(platonicScene, cam);
  const bloomPass  = new UnrealBloomPass(new THREE.Vector2(W, H), 0.25, 0.5, 0.1);
  const composer   = new EffectComposer(renderer);
  composer.addPass(renderPass);
  composer.addPass(bloomPass);

  let activeCategory = 'platonic';
  let activeScene    = platonicScene;
  ...
  populateShapes('platonic');
  shapeSel.value = '2';  // Dodecahedron / Icosahedron
  platonicUniforms.u_pair.value = 2;
  ...
  soundSel.value = 'kick2';
  composer.render();
  loadSound(SOUND_FILES.find(x => x.value === 'kick2'));
}
