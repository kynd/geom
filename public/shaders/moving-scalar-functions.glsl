float surfaceF(vec3 p) {
  ...
  // 1. Traveling radial damped cosine — damped radial wave whose phase advances with time
  if (u_surfaceIndex == 1) {
    float r = length(vec2(x, z));
    return y - 0.52*exp(-r*1.1)*cos(5.5*r - 2.2*t);
  }

  // 2. Pulsing torus — major radius breathes while azimuthal waves travel around the tube
  if (u_surfaceIndex == 2) {
  ...
  // 4. Oscillating spheroid — quadrupolar mode: alternates between prolate and oblate via P₂(cosθ)
  if (u_surfaceIndex == 4) {
    float rlen = length(q);
  ...
  // 5. Pulsing saddle — amplitude of x²−z² breathes
  if (u_surfaceIndex == 5) {
    float A = 0.70 + 0.30*sin(t*0.75);
    return y - A*(x*x - z*z);
  }

  // 6. Pulsing gyroid — triply periodic minimal surface; spatial scale k(t) oscillates
  if (u_surfaceIndex == 6) {
    float k = 1.45 + 0.42*sin(t*0.50);
    return cos(k*x)*sin(k*y) + cos(k*y)*sin(k*z) + cos(k*z)*sin(k*x);
  }

  // 7. Oscillating Schwartz P — triply periodic minimal surface; threshold c(t) shifts
  if (u_surfaceIndex == 7) {
    float c = 0.88*sin(t*0.55);
    return cos(2.0*x) + cos(2.0*y) + cos(2.0*z) - c;
  }

  // 8. Pulsing lemniscate — (x²+y²+z²)² = a²(x²−z²); axis a(t) oscillates
  if (u_surfaceIndex == 8) {
  ...
  // 9. Tilting ellipsoid — elongated along Y; long axis tilts while axes breathe
  if (u_surfaceIndex == 9) {
  ...
  // 10. Pulsing tanglecube — degree-4 surface; constant c(t) slides the isosurface
  ...
  // 11. Pulsing Chmutov T₄ — sum of 4th Chebyshev polynomials; scale s(t) oscillates
  ...
  // 12. Traveling sinusoidal cone — cone with a sinusoidal wave traveling along the axis
  ...
  // 15. Tilting saddle — cross-term A(t)·xz rotates the principal curvature axes
  if (u_surfaceIndex == 15) {
    float A = 1.20*sin(t*0.65);
    return y - 0.72*(x*x - z*z + A*x*z);
  }

  // 16. Rotating torus — tube radius modulated sinusoidally around the ring angle
  ...
  // 18. Traveling hyperboloid — sinusoidal wave modulates the xz cross-section
  if (u_surfaceIndex == 18) {
    float A = 0.28*sin(t*0.72);
    return x*x*(1.0 + A*sin(2.8*y - t)) + z*z - y*y - 0.40;
  }

  // 19. Pulsing cyclic cubic — cyclic cubic x²y + y²z + z²x = A(t)
  if (u_surfaceIndex == 19) {
    float A = 0.20 + 0.14*sin(t*0.82);
    return x*x*y + y*y*z + z*z*x - A;
  }

  // 20. Rotating paraboloid — a=sin, b=cos at same frequency: cycles bowl→saddle→inverted bowl→saddle
