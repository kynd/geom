...
const MODE_DESCS = [
  ...
  '128 FFT bins as concentric glowing rings; radius encodes frequency, hue follows radius and time.',
  ...
  'Bass at top, treble at bottom. L channel on left arc, R channel on right arc (mirrored). Centre = now, data expands outward — edge starts black (future frames).',
  '128 FFT bins as concentric rings; hue encodes amplitude — cool blue for quiet bins, warm orange for loud.',
  '128 FFT bins as concentric rings; both hue and lightness encode amplitude — quiet bins are dark and cool, loud bins are bright and warm.',
];
...
const waveArray = new Uint8Array(FFT_BINS * 2); // fftSize = 512

// Texture data (RGBA, R channel carries the data)
...
async function init() {
  ...
  trackSelect.addEventListener('change', async () => {
    ...
    if (isPlaying) {
      const url = `../../../sound/highlights/${TRACKS[currentTrackIdx].file}`;
      await loadAndPlayTrack(url);
    } else {
      trackChanged = true;
    }
  });
