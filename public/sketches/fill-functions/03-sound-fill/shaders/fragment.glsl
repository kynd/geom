...
// 128 FFT bins as concentric glowing rings; radius encodes frequency.
// Hue is purely radial (no theta term) to avoid the seam at atan discontinuity.
vec3 mode4(vec2 uv) {
  float r      = length(uv);
  float bright = 0.0;
  for (int k = 0; k < 128; k++) {
    ...
    float amp     = sampleFFT(freq);
    float targetR = 0.05 + freq * 0.95;
    float d       = (r - targetR) / 0.004;
    bright += amp * exp(-d * d);
  }
  bright = clamp(bright, 0.0, 1.0);
  if (bright < 0.002) return vec3(0.0);
  float H = mod(r * PI + iTime * 0.08, 2.0 * PI);
  float L = 0.08 + 0.72 * bright;
  return lch(L, maxChroma(L, H) * bright * 0.92, H);
}

// ── Mode 10 — Radial spectrum · hue from amplitude ───────────────────────────
// All 128 rings always visible at constant brightness. Hue encodes amplitude:
// cool blue for quiet bins, warm orange for loud bins.
// Weighted by proximity only (not amplitude), so quiet rings still show.
vec3 mode10(vec2 uv) {
  float r = length(uv);
...
// ── Mode 11 — Radial spectrum · hue + L from amplitude ───────────────────────
// All 128 rings structurally visible (proximity weighting). Both hue and
// lightness encode amplitude: quiet bins are dark and cool, loud bins bright and warm.
vec3 mode11(vec2 uv) {
  float r = length(uv);
...
// Rotated so bass sits at top. L channel on left arc, R channel on right arc (mirrored).
// Radius = time: centre = now, edge = oldest. Hue rotates slowly (10-second cycle).
vec3 mode9(vec2 uv) {
  float rawR  = length(uv);
  if (rawR > 1.0) return vec3(0.0);
  float r     = max(rawR, 0.001);
  float theta = mod(atan(uv.y, uv.x) - PI * 0.5 + 2.0 * PI, 2.0 * PI);
  float age   = r * 255.0;
...
vec3 fill(vec2 uv, vec2 nUV) {
  ...
  if (u_mode == 9) return mode9(uv);
  if (u_mode == 10) return mode10(uv);
  return mode11(uv);
}
