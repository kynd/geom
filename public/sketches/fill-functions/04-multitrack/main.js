...
const FPS            = 60;
const FFT_BINS       = 128;
const HISTORY_FRAMES = 900;   // 15 s rolling window, matching sound-analysis
...
// Rolling-window amplitude history: grows from left, scrolls once full.
// Matches the sound-analysis demo behaviour exactly.
...
async function init() {
  ...
  // Audio — master only, streams in background
  ...
  function draw() {
    ...
    for (let t = 0; t < N; t++) {
      ...
      // Amplitude history: R under L, rolling window
      ...
      // Track label — Sora (label font), smaller weight
