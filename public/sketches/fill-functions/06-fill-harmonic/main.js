...
const MODE_NAMES = [
  'Spectrogram', 'Phase portrait', 'Wave · anisotropic', 'Crystal · drift',
  'Radial spectrum', 'Waveform ring', 'Cosine product', 'Polar harmonics',
  'Phase modulation', 'Circular spectrogram',
];
const MODE_DESCS = [
  'Frequency on x, time scrolling downward. Hue maps across the full 360° wheel from bass to treble.',
  'Lissajous figure from synthesized waveform. All 128 Mel bins drive harmonic oscillators at frequencies 1f–128f; their superposition traces curves that twist and fold with the full spectral content.',
  ...
  'Ring radius deformed by a synthesized waveform. All 128 Mel bins contribute harmonics — the full spectral shape bends the ring, so melodic content draws smooth petals and transients spike it.',
  ...
  'Radial rings phase-modulated by a synthesized waveform. Harmonic synthesis bends rings in proportion to the spectral energy at each frequency.',
...
let allFrames  = [];
let audio      = null;
let currentMel = new Array(128).fill(0);
let audioCtx   = null;
let analyser   = null;
const rawFft   = new Uint8Array(512);
...
// Harmonic synthesis: all 128 Mel bins as harmonically-spaced oscillators.
// Bin k drives a sine at harmonic k+1; fixed per-harmonic offset k staggers
// oscillators without time-based drift, so the shape tracks the spectrum directly.
function synthesizeWav(mel) {
  const N = FFT_BINS * 2;
  for (let n = 0; n < N; n++) {
    let w = 0;
    for (let k = 0; k < 128; k++) {
      w += melDB(mel[k]) * Math.sin(2 * Math.PI * (k + 1) * n / N + k * 1.0);
    }
    wavBuf[n * 4]     = Math.max(0, Math.min(255, 128 + Math.round(w * 5)));
    wavBuf[n * 4 + 3] = 255;
  }
}
...
function tapAudio(audioEl) {
  if (!audioCtx || !audioEl) return;
  try { audioCtx.createMediaElementSource(audioEl).connect(analyser); } catch (e) {}
}

async function init() {
  const canvas    = document.getElementById('canvas');
  const W = canvas.width, H = canvas.height;
  const cmpCanvas = document.getElementById('fft-cmp');
  const cmpCtx    = cmpCanvas.getContext('2d');
  cmpCanvas.width  = W; cmpCanvas.height  = H;
  cmpCanvas.style.width = W + 'px'; cmpCanvas.style.height = H + 'px';
  ...
  function play() {
    if (!audio) return;
    if (!audioCtx) { ensureAudioCtx(); tapAudio(audio); }
  ...
  function drawComparison() {
    const bottomOff = Math.round(H * 0.15); // clear the mode name + description area
    const rowH   = Math.round(H * 0.11);
    const melY   = H - bottomOff - rowH * 2; // Mel row: top
    const fftY   = H - bottomOff - rowH;     // FFT row: bottom
    const barH   = rowH - 18;

    cmpCtx.clearRect(0, 0, W, H);

    // Mel row background
    cmpCtx.fillStyle = 'rgba(0,0,0,0.52)';
    cmpCtx.fillRect(0, melY, W, rowH);

    // FFT row background (slightly different shade to distinguish)
    cmpCtx.fillStyle = 'rgba(0,0,0,0.58)';
    cmpCtx.fillRect(0, fftY, W, rowH);

    // Mel 128 bands (pre-computed .bin) — blue, top row
    // Apply same dB mapping as getByteFrequencyData: minDecibels=-100, maxDecibels=-30
    const mbw = W / 128;
    for (let i = 0; i < 128; i++) {
      const db = 20 * Math.log10(Math.max(currentMel[i] || 0, 1e-5));
      const v  = Math.max(0, Math.min(1, (db + 80) / 80));
      const bh = v * barH;
      cmpCtx.fillStyle = `rgba(90,170,255,${(0.30 + v * 0.60).toFixed(2)})`;
      cmpCtx.fillRect(i * mbw, melY + rowH - bh, Math.max(2, mbw - 1), bh);
    }

    // FFT 512 bins (real-time) — white, bottom row
    if (analyser) {
      analyser.getByteFrequencyData(rawFft);
      const bw = W / 512;
      for (let i = 0; i < 512; i++) {
        const v  = rawFft[i] / 255;
        const bh = v * barH;
        cmpCtx.fillStyle = `rgba(255,255,255,${(0.40 + v * 0.50).toFixed(2)})`;
        cmpCtx.fillRect(i * bw, fftY + rowH - bh, Math.max(1, bw - 0.3), bh);
      }
    }

    cmpCtx.font = "11px 'Sora', sans-serif";
    cmpCtx.fillStyle = 'rgba(90,170,255,0.90)';
    cmpCtx.fillText('Mel · 128 bands (pre-computed .bin)', 12, melY + 13);
    cmpCtx.fillStyle = 'rgba(255,255,255,0.65)';
    cmpCtx.fillText('FFT · 512 linear bins (real-time)', 12, fftY + 13);
  }
