...
// Band-split oscillators: bass, mid, and treble each drive a separate sine at
// a distinct frequency. The three components sum into a compound waveform whose
// shape changes character depending on which frequency band dominates — low
// frequencies produce slow, wide undulations; treble adds tight ripple on top.
function synthesizeWav(bass, mid, treble, iTime) {
  const N = FFT_BINS * 2;
  for (let n = 0; n < N; n++) {
    const t = n / (N - 1);
    const w = bass   * Math.sin(2 * Math.PI * t       + iTime * 1.1)
            + mid    * Math.sin(2 * Math.PI * t * 7   + iTime * 2.7)
            + treble * Math.sin(2 * Math.PI * t * 23  + iTime * 5.1);
    wavBuf[n * 4]     = Math.max(0, Math.min(255, 128 + Math.round(w * 95)));
    wavBuf[n * 4 + 3] = 255;
  }
}
...
function updateFromPrecomputed(frame, uniforms, textures) {
  ...
  synthesizeWav(bass, mid, treble, uniforms.iTime.value);
