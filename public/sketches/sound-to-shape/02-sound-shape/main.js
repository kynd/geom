...
const FPS  = 60;
const HIST = 256;
const PLAT_CYCLE = 5.0;  // seconds per morph cycle
...
async function init() {
  ...
  const [
    ...
    fetch('./shaders/fragment-moving.glsl').then(r => r.text()),
    ...
    fetch('./shaders/deform.glsl').then(r => r.text()),
  ]);

  // Rename surfaceF → baseScalarF in moving-scalar-functions so we can wrap it
  ...
  // Shared dynamic uniforms (same object, referenced by all materials)
  const sharedUniforms = {
    ...
    u_sssStr:     { value: 0.3 },
    // Per-effect deformation parameters
    u_deformP1:      { value: 0.38 },  // inflate default; re-synced on mode change
    u_deformP2:      { value: 0.55 },  // squash height default
    // History controls (modes 4–6)
    u_histDuration:  { value: 1.0  },
    u_histSoften:    { value: 0.0  },
    // Twist axis (mode 6)
    u_twistAxisX:    { value: 0.0  },
    u_twistAxisZ:    { value: 0.0  },
    // Spectrum spline control points (mode 8)
    u_ctrlN:         { value: 2.0  },
  };

  // Per-category uniforms (merged with shared)
  ...
  const movingUniforms   = { ...sharedUniforms, u_surfaceIndex: { value: 1  } };
  ...
  // Bloom is applied on top of whatever scene is active
  const renderPass = new RenderPass(movingScene, cam);
  const bloomPass  = new UnrealBloomPass(new THREE.Vector2(W, H), 0.25, 0.5, 0.1);
  const composer   = new EffectComposer(renderer);
  composer.addPass(renderPass);
  composer.addPass(bloomPass);

  // ── shape selection state ──────────────────────────────────────────────────
  // category: 'sdf' | 'platonic' | 'scalar' | 'moving'
  ...
  function loop(ts) {
    ...
    // Animate platonic morph parameter
  ...
  syncDeformParams(1);  // init param panel for default inflate mode
