...
// Scene: a gently rotating rounded box
...
float sceneSDF(vec3 p) {
  ...
  return sdRoundBox(p, vec3(0.42), 0.10);
}
...
vec3 render(vec2 fragCoord) {
  ...
  // Screen polar: map angle to FFT bin for warp magnitude
  float screenAng = atan(uv.y, uv.x) / 6.28318 + 0.5;
  float fftVal    = fftAt(screenAng) * u_amp;

  // Warp direction: rotate with time so it sweeps around
  float warpAng = screenAng * 6.28318 + iTime * 0.40;

  // Radial distance from center on screen — warp stronger at edges
  float screenR = length(uv) / 1.778;

  // Warp the ray direction: twist it in XY by a rotation proportional to fftVal
  float warpMag  = fftVal * 0.55 * (0.5 + screenR);
  float wx = cos(warpAng) * warpMag;
  float wy = sin(warpAng) * warpMag;

  vec3 ro = vec3(0.0, 0.0, 3.2);
  // Base ray, then add warp perpendicular to Z
  vec3 rd = normalize(vec3(uv.x + wx, uv.y + wy, -3.0));
